...
public:
	...
	short dataTypeCheck(string pSyntax, vector<DataType> &pVDataType);
	short sequenceTypeCheck(string pSyntax, vector<Sequence> &pVSequence, vector<DataType> &pVDataType, string pValue);

	int checkValueSize(string pValue, string pName, vector<ObjectTypeSize>& pVObjectTypeSize, vector<DataType> &pVDataType, vector<Sequence> &pVSequence);
